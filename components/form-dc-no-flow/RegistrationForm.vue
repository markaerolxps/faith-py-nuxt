<template>
    <form class="flex flex-col items-start justify-center w-full " name="register-form" method="POST"
        ...
        <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
            <Dropdown :title="'Filipino?'" :items="defaultItems" :hasOption="false" @selected-type="selectedFilipinoAction"
                v-model="filipinoActionSelected" />

        </div>
        <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
            v-if="filipinoActionSelected === 'Yes'">
            <Dropdown :title="'Have you submitted your child’s birth certificate?'" :items="defaultItems" :hasOption="false"
                @selected-type="selectedBirthCertAction" v-model="birthCertActionSelected" />
        </div>
        <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
            v-if="birthCertActionSelected === 'No' && filipinoActionSelected === 'Yes'">
            <UploadFile :title="'Upload Birth Certificate'" @uploadFile="birthCertUploadData"
                :refName="'uploadBirthCert'" />

        </div>
        <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
            v-if="birthCertActionSelected != 'Select' && filipinoActionSelected === 'Yes'">
            <Dropdown :title="'Does your child have a Philippine Passport?'" :items="defaultItems" :hasOption="false"
                @selected-type="selectedPhPassportAction" v-model="phPassportActionSelected" />


        </div>

        <div class="flex flex-col w-full mb-4 gap-1" v-if="birthCertActionSelected != 'Select' && filipinoActionSelected === 'Yes' &&
            ...
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                ...
                <InputText v-model="passportDate" :title="'Passport Expiration Date'"
                    :placeholder="'Passport Expiration Date'" v-on:change="inputChange" :type="'date'"
                    :name="'passportDate'" :value="passportDate" />
            </div>
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                <UploadFile :title="'Upload Philippine Passport'" @uploadFile="uploadPassport"
        ...
        <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
            ...
            <div class="w-full flex flex-col mb-4 items-start">
                <Dropdown :title="'Country'" :items="countries" :hasOption="true" @selected-type="selectedCountry"
            ...
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
                ...
                <InputText v-model="otherCountry" :title="'Other'" :placeholder="'Other'" v-on:change="inputChange"
                    :type="'text'" :name="'otherCountry'" :value="otherCountry" />

            </div>
            <div class="w-full flex flex-col mb-4 items-start">
                <Dropdown :title="'Nationality'" :items="nationalities" :hasOption="true"
                    @selected-type="selectedNationality" v-model="nationalitySelected" />

            </div>
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active"
                v-if="nationalitySelected === 'Other'">

                <InputText v-model="otherNationality" :title="'Other'" :placeholder="'Other'" v-on:change="inputChange"
                    :type="'text'" :name="'otherNationality'" :value="otherNationality" />

            </div>
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                <InputText v-model="surName" :title="'Surname'" :placeholder="'Surname'" v-on:change="inputChange"
                    :type="'text'" :name="'surName'" :value="surName" />
            </div>

            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                <InputText v-model="firstName" :title="'First Name'" :placeholder="'First Name'" v-on:change="inputChange"
            ...
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                <InputText v-model="middleName" :title="'Middle Name'" :placeholder="'First Name'" v-on:change="inputChange"
                    :description="'If you don’t have one, just type “none”'" :type="'text'" :name="'middleName'"
                    :value="middleName" />
            ...
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                ...
                <InputText v-model="birthPlace" :title="'Birth Place'" :placeholder="'Birth Place'"
                    v-on:change="inputChange" :type="'text'" :name="'birthPlace'" :value="birthPlace" />

            </div>

            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                <InputText v-model="dateOfBirth" :title="'Date of Birth'" :placeholder="'Date of Birth'"
                    v-on:change="inputChange" :type="'date'" :name="'dateOfBirth'" :value="dateOfBirth" />
            </div>
            ...
            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                ...
                <InputText v-model="issuingAuthority" :title="'Issuing Authority'" :placeholder="'Issuing Authority'"
                    v-on:change="inputChange" :type="'text'" :name="'issuingAuthority'" :value="issuingAuthority" />

            </div>

            <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                <UploadFile :title="'Upload Passport Bio Page'" @uploadFile="uploadPassportBio"
                    :refName="'uploadPassportBio'" />

            </div>

            <div class="w-full flex flex-col mb-4 items-start">
                <Dropdown :title="'Is your child currently in the Philippines?'" :items="defaultItems" :hasOption="false"
                    @selected-type="selectedChildCurrentlyInPh" v-model="childCurrentlyInPhSelected" />

            </div>

            <div class="flex flex-col items-start w-full" v-if="childCurrentlyInPhSelected !== 'Select'">

                <div class="flex flex-col items-start w-full" v-if="childCurrentlyInPhSelected === 'Yes'">
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                        <InputText v-model="dateOfArrival" :title="'Date of Arrival'" :placeholder="'Date of Arrival'"
                            v-on:change="inputChange" :type="'date'" :name="'dateOfArrival'" :value="dateOfArrival" />

                    </div>

                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                        <InputText v-model="dateOfAuthorizedStay" :title="'Date of Authorized Stay'"
                            :placeholder="'Date of Authorized Stay'" v-on:change="inputChange" :type="'date'"
                            :name="'dateOfAuthorizedStay'" :value="dateOfAuthorizedStay" />
                    </div>


                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                        <UploadFile :title="'Upload Arrival Stamp'" @uploadFile="uploadArrivalStamp"
                            :refName="'uploadArrivalStamp'" />
                    </div>

                </div>

                <div class="flex flex-col w-full items-start" v-if="childCurrentlyInPhSelected === 'No'">
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                        <InputText v-model="anticipatedArrivalDate" :title="'Anticipated Date of Arrival'"
                            :placeholder="'Anticipated Date of Arrival'" v-on:change="inputChange" :type="'date'"
                            :name="'anticipatedArrivalDate'" :value="anticipatedArrivalDate" />

                    </div>
                </div>

                <div class="w-full flex flex-col mb-4 items-start">
                    <Dropdown :title="'How did/will your child enter the Philippines?'" :items="childEnterReasonItems"
                        :hasOption="false" @selected-type="selectedChildEnterReason" v-model="childEnterReasonSelected"
                        :isObjectArray="true" />

                </div>


                <div class="w-full flex flex-col mb-4 items-start" v-if="childEnterReasonSelected === 'balikbayan-visa' || childEnterReasonSelected === 'visa-granted'
                    || childEnterReasonSelected === 'visa-free'">
                    <Dropdown :title="'Does your child need a visa to study in the Philippines?'" :items="childVisaItems"
                        :hasOption="false" @selected-type="selectedChildVisa" v-model="childVisaSelected"
                        :isObjectArray="true" />
                </div>

                <div class="w-full flex flex-col mb-4 items-start rounded-2xl bg-[#F0F7FE] justify-center "
                    style="padding: 16px 24px;"
                    v-if="(childEnterReasonSelected != 'ph-visa' && childEnterReasonSelected != 'visa-holder') && childVisaSelected != 'Select'">

                    <div class="flex flex-col w-full items-start justify-center gap-2"
                        v-if="childVisaSelected === 'fa-a2-visa' || childVisaSelected === 'balikbayan-visa-fa'">

                        <span class="text-sm font-medium text-[#1C355E]">Apply for 47(a)(2)
                            Visa</span>

                        <div class="w-full flex flex-col items-start px-2 gap-2">
                            <span class="text-xs font-normal ">We will prepare your child's application.
                                Expect instructions to be sent to you via email.Please turnover your
                                passport immediately to the Visa Office at Faith Academy or as soon as
                                you arrive in the country</span>

                            <span class="text-xs font-normal ">For more information please email
                                g.visaoffice@faith.edu.ph</span>
                        </div>

                    </div>
                    <div class="flex flex-col w-full items-start justify-center gap-2"
                        v-if="childVisaSelected === 'fix-balikbayan-visa' || childVisaSelected === 'fix-ph-visa'">

                        <span class=" text-sm font-medium text-[#1C355E]">Apply for SSP</span>
            ...
            <div class="flex flex-col items-start w-full " v-if="childEnterReasonSelected === 'ph-visa'">

                <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                    <InputText v-model="typeOfPhVisa" :title="'Type of Philippine Visa'"
                        :placeholder="'Type of Philippine Visa'" v-on:change="inputChange" :type="'text'"
                        :name="'typeOfPhVisa'" :value="typeOfPhVisa" />
                </div>

                <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">

                    <InputText v-model="phVisaExpDate" :title="'Expiration Date of Visa'"
                        :placeholder="'Expiration Date of Visa'" v-on:change="inputChange" :type="'date'"
                        :name="'phVisaExpDate'" :disabled="inDefinite ? true : false" :value="phVisaExpDate" />

                    <div class="flex flex-row items-center justify-start mt-2 gap-1">
                        <input type="checkbox" name="inDefinite" id="inDefinite" v-model="inDefinite" class="  rounded-lg "
                            @input="inputChange" />
                ...
                <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                    <UploadFile :title="`Upload your child's visa page`" @uploadFile="uploadChildsVisaPage"
                        :refName="'uploadChildsVisaPage'" />

                </div>

                <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                    <Dropdown :title="'Do you have an ACR I-card or Philippine Retirement Authority (PRA) Card?'"
                        :items="defaultItems" :hasOption="false" @selected-type="selectedAcrCardAction"
                        v-model="acrCardActionSelected" />

                </div>
                <div class="flex flex-col items-start w-full " v-if="acrCardActionSelected === 'Yes'">
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                        <InputText v-model="acrCardExpDate"
                            ...
                            v-on:change="inputChange" :type="'date'" :name="'acrCardExpDate'" :value="acrCardExpDate" />
                    </div>
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                        <UploadFile
                            :title="`Upload your child's valid card, front ACR I-card or Philippine Retirement Authority (PRA) Card`"
                            @uploadFile="uploadAcrCardFront" :refName="'uploadAcrCardFront'" />
                    ...
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
                        <UploadFile
                            ...
                            @uploadFile="uploadAcrCardBack" :refName="'uploadAcrCardBack'" />

                    </div>
                </div>
                <div class="flex flex-col items-start w-full " v-if="acrCardActionSelected === 'No'">
                    <div class="flex flex-col items-start w-full gap-1 mb-4 fade-up-enter-active">
    ...
    methods: {
        ...
        getFormValues() {
            if (this.formData) {
                this.formData.forEach(element => {
                    ...
                    this.childCurrentlyInPhSelected = element.childCurrentlyInPh || 'Select'
        ...
        submitStateValidate() {
            if (this.phPassportActionSelected === 'No' && this.birthCertActionSelected === 'Yes' && this.filipinoActionSelected === 'Yes' && this.recaptchaToken) {
                ...
                && ((this.nationalitySelected != 'Select' && this.nationalitySelected != 'Other') || (this.nationalitySelected === 'Other' && this.otherNationality))
                ...
                && this.surName && this.middleName && this.firstName && this.birthPlace && this.dateOfBirth && this.passportNumber
                && this.passportDate && this.issuingAuthority && this.passportBioFile != null && this.childCurrentlyInPhSelected === 'No' && this.anticipatedArrivalDate
                && this.childEnterReasonSelected === 'ph-visa' && this.typeOfPhVisa && (this.phVisaExpDate || this.inDefinite)
                ...
                && ((this.nationalitySelected != 'Select' && this.nationalitySelected != 'Other') || (this.nationalitySelected === 'Other' && this.otherNationality))
                && this.surName && this.middleName && this.firstName && this.birthPlace && this.dateOfBirth && this.passportNumber
                && this.passportDate && this.issuingAuthority && this.passportBioFile != null && this.childCurrentlyInPhSelected === 'No' && this.dateOfArrival
                && this.dateOfAuthorizedStay && this.arrivalStampFile != null && this.childEnterReasonSelected === 'ph-visa' && this.typeOfPhVisa && (this.phVisaExpDate || this.inDefinite)
        ...
        sendForm() {
            ...
            if (this.flowFormat === 'flow-5') {
                formRequest.push({
                    ...
                    otherNationality: this.countrySelected === 'Other' ? this.otherNationality : null,
