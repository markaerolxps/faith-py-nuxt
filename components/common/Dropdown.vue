    ...
    <div class="flex flex-col items-start w-full gap-1 relative">
        ...
        <div :class="`bg-[#F5F5F5] rounded-lg w-full items-start flex flex-col justify-start relative ${isError && 'border-2 border-red-400'}`" id="fil-action"
            ...
            <div  :class="`flex flex-row justify-between w-full z-20`" style="padding: 8px 12px;" id="fil-field">
                ...
                <span v-if="isObjectArray" class="font-normal text-sm " id="text-reason"
                    :class="modelValue !== 'Select' ? 'text-[#252525]' : 'text-[#b8b8b8]'">{{ arrayTextName }}</span>
        ...
        <div class="relative flex w-full z-50 " v-if="showMenu">
            ...
            <div class="flex flex-col absolute w-full items-start  gap-1 " v-if="isObjectArray"
                style="border-radius: 8px;border: 1px solid var(--neutral-5, #D9D9D9);background: #FFF;box-shadow: 0px 2px 12px 0px rgba(0, 0, 0, 0.10);padding: 8px 0px 8px 1px;">
                <div v-for="(item, index ) in items" :key="index" class="flex flex-col w-full">
                    <span v-if="!item.link" class="text-xs font-normal px-2 w-full hover:bg-[#F5F5F5] cursor-pointer"
                        @click="selectedType(item)">{{ item.name }}</span>
                    <div class="w-full flex flex-col hover:bg-[#F5F5F5] items-start" v-if="item.link">
                        <span class="text-xs font-normal px-2 w-full  cursor-pointer" @click="selectedType(item)">{{
                            item.name }}</span>
                        <a class="text-xs font-bold px-2 w-full  cursor-pointer" :href="item.link" target="__blank">{{
                            item.link }}</a>
                    </div>
                </div>
            </div>
