  <template>
    <div :class="`flex flex-col items-start w-full gap-1 ${classParentContainer}`">
      <div class="flex-row items-start w-full gap-1">
        <label class="text-sm font-normal">{{ title }}</label>
        <span v-if="description != 'Indefinite'" class="text-sm font-normal text-[#FF4D4F]">*</span>
      ...
      <div :class="description != 'Indefinite' ? 'w-full' : ''">
        <input v-bind="$attrs"
          :class="`border px-[12px] py-[8px] border-[#D9D9D9] bg-[#F5F5F5] rounded-lg w-full font-normal text-sm ${classInput} ${isError && 'border-2 border-red-400'}`"
          @input="updateValue" />
      </div>
    </div>
  </template>
